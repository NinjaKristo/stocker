...
vi.mock('./createPriceChartSeries', () => ({
  createPriceChartSeries: () => mocks.bundle,
}));

describe('CandlestickChart delayed intraday mode', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('loads five-minute bars and labels them as delayed with the actual bar time', async () => {
    const user = userEvent.setup();
    renderWithProviders(<CandlestickChart symbol="AAPL" height={400} />);

    await screen.findByText(/Data through Jul 16, 2026/);
    await user.click(screen.getByRole('button', { name: '5 min delayed' }));

    expect(await screen.findByText(/Delayed 5 min.*Yahoo Finance.*bar Jul 17/)).toBeInTheDocument();
    expect(mocks.fetchIntradayPriceHistory).toHaveBeenCalledWith('AAPL', '5m');
    ...
    await waitFor(() => {
      ...
      expect(latestSet[0].time).toBe(Math.floor(Date.parse('2026-07-17T15:55:00-04:00') / 1000));
    });
  });
});
