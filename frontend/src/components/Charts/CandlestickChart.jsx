...
function CandlestickChart({
  ...
  const shouldFitModeRef = useRef(false); // Daily and hourly modes use incompatible time axes
  ...
  const intradaySelected = !priceData && timeframe === 'hourly';
  const {
    ...
    queryKey: priceHistoryKeys.intraday(symbol, '60m'),
    queryFn: () => fetchIntradayPriceHistory(symbol, '60m'),
      ...
      {!showLoading && !showError && !showNoData && (intradaySelected ? latestIntradayBarText : dataThroughText) && !showRefreshIndicator && (
        ...
        >
          <Typography variant="caption" sx={{ color: '#999', fontSize: '0.65rem' }}>
            {intradaySelected
              ? `Hourly delayed · ${intradayResponse?.source || 'public source'} · bar ${latestIntradayBarText}`
