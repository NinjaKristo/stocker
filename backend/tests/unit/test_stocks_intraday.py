...
@pytest.mark.asyncio
async def test_intraday_endpoint_returns_hourly_delayed_contract(client, monkeypatch):
    ...
    response = await client.get("/api/v1/stocks/aapl/intraday", params={"interval": "60m"})

    assert response.status_code == 200, response.text
    assert response.json()["is_realtime"] is False
    assert service.calls == [("AAPL", "60m")]
...
async def test_intraday_endpoint_reports_provider_unavailability(client, monkeypatch):
    service = _FakeIntradayService(
        error=IntradayPriceUnavailable("Delayed hourly data is unavailable for AAPL")
    )
    monkeypatch.setattr(stocks_module, "_get_intraday_price_service", lambda: service)
