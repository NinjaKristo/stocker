...
def test_fetches_timestamped_hourly_bars_and_caches_payload():
    provider = _FakeYFinance(_frame())
    redis = _FakeRedis()
    service = IntradayPriceService(yfinance_service=provider, redis_client=redis)

    payload = service.get("aapl", interval="60m")

    assert payload["symbol"] == "AAPL"
    assert payload["is_realtime"] is False
    assert payload["cache_status"] == "miss"
    assert payload["latest_bar_at"] == "2026-07-17T10:30:00-04:00"
    assert provider.calls == [
        ("AAPL", {"period": "1mo", "interval": "60m", "use_cache": False})
    ]
...
def test_redis_hit_avoids_another_provider_request():
    ...
    assert service.get("MSFT")["cache_status"] == "miss"
    assert service.get("MSFT")["cache_status"] == "hit"
    assert len(provider.calls) == 1
    assert json.loads(redis.values["intraday:v1:MSFT:60m"])["symbol"] == "MSFT"


def test_malformed_provider_frame_fails_honestly():
    service = IntradayPriceService(
        yfinance_service=_FakeYFinance(pd.DataFrame({"Close": [1.0]}))
    )
