...
class IntradayPriceService:
    """Fetch and briefly cache delayed hourly Yahoo Finance bars."""

    CACHE_VERSION = "v1"
    CACHE_TTL_SECONDS = 60
    PERIOD_BY_INTERVAL = {"60m": "1mo"}

    def __init__(self, *, yfinance_service, redis_client=None):
        self._yfinance_service = yfinance_service
        self._redis_client = redis_client

    def get(self, symbol: str, *, interval: str = "60m") -> dict[str, Any]:
        ...
        if not bars:
            raise IntradayPriceUnavailable(
                f"Delayed hourly data is unavailable for {normalized_symbol}"
            )
