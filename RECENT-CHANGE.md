...
| Added `HR | DAY | WEEK | RS` to the shared chart | The requested compact hourly-first control was missing | Ordered and relabeled the existing controls, with `HR` backed by React Query and a 60-second stale time | Component test verifies the exact control order and the `60m` endpoint call |
